...
RUN \
  ...
  curl -s -o /nginx-sources.tar.gz http://nginx.org/download/nginx-1.9.15.tar.gz && \
  tar xzf /nginx-sources.tar.gz && \
  git clone https://github.com/mailru/graphite-nginx-module.git && \
  cd /nginx-1.9.15 && patch -p1 < /graphite-nginx-module/graphite_module_v1_7_7.patch && \
  ./configure --with-http_stub_status_module --with-ipv6 --with-http_ssl_module --add-module=/graphite-nginx-module && make && make install && \
  useradd -m nginx && \
  chown :33 /usr/local/nginx && \
  chmod 0770 /usr/local/nginx && \
  rm -rf /nginx-1.9.15 && rm -rf /nginx-sources.tar.gz && \
